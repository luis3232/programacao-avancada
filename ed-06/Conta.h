...
public:
	...
	Conta(std::string numero, std::string agencia, Cliente *cliente, float saldo) : _numero(numero), _agencia(agencia), _cliente(cliente), _saldo(saldo) {
		std::cout << "\nCriando conta do cliente " << _cliente->getNome() << ", com os seguintes dados: \n" << std::endl;
		std::cout << "Agencia: " << _agencia << std::endl;
		std::cout << "Numero: " << _numero << std::endl;
	}
